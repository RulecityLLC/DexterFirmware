...
void vp931_main_loop()
{
	...
	while (!g_bRestartPlayer)
	{
		...
		/*
		NOTE: I tried converting this over to using common-ldp but it didn't quite work.
		Issues include vp931-callbacks doing its own seek instead of using common-ldp's seek.
		I reverted because it didn't seem worth it to try to make it work when the only reason I was trying to use common-ldp was to get better stop-code support (which FFX and FFR probably never use).
		*/
